...
class ProfileEdit extends StatefulWidget {
  ...
  const ProfileEdit({Key? key, required this.profileProvider}) : super(key: key);
...
class _ProfileEditState extends State<ProfileEdit> {
              ...
              GestureDetector(
                onTap: () {
                      ...
                      setState(() {
                        ...
                        if(profileProvider.profileModel?.data?.profile?.image != null && profileProvider.profileModel?.data?.profile?.image != '') {
                          profileProvider.deleteProfilePic(profileProvider.profileModel?.data?.profile?.patientNoPk ?? 0).then((value) {
                            if(value) {
  ...
  void initState() {
    ...
    _firstNameETController.text = widget.profileProvider.profileModel?.data?.profile?.firstName ?? "";
    _lastNameETController.text = widget.profileProvider.profileModel?.data?.profile?.lastName ?? "";
    _emailETController.text = widget.profileProvider.profileModel?.data?.profile?.email ?? "";
    _phoneETController.text = widget.profileProvider.profileModel?.data?.profile?.mobile ?? "";
    _dprIdETController.text = widget.profileProvider.profileModel?.data?.profile?.dprId ?? "";
    //date of birth
    _dob = widget.profileProvider.profileModel?.data?.profile?.dob?.split(" ").first ?? '';
    if(_dob != '') {
    ...
    _ageETController.text = widget.profileProvider.profileModel?.data?.profile?.age ?? "";
    _heightFootETController.text = widget.profileProvider.profileModel?.data?.profile?.height?.split('.').first != '0' ? (widget.profileProvider.profileModel?.data?.profile?.height?.split('.').first ?? '') : '';
    _heightInchETController.text = (widget.profileProvider.profileModel?.data?.profile?.height ?? "").contains(".") ? widget.profileProvider.profileModel!.data!.profile!.height!.split('.').last : "";
    _weightETController.text = widget.profileProvider.profileModel?.data?.profile?.weight?.toString() ?? "";
    _addressETController.text = widget.profileProvider.profileModel?.data?.profile?.address ?? "";
    _selectedGenderId = widget.profileProvider.profileModel?.data?.profile?.gender != null ? _genderList.firstWhere((gender) => gender.gender == widget.profileProvider.profileModel?.data?.profile?.gender).id : null;
    _selectedMaritalStatusId = widget.profileProvider.profileModel?.data?.profile?.maritalStatus != null ? _maritalStatusList.firstWhere((maritalStatus) => maritalStatus.maritalStatus == widget.profileProvider.profileModel?.data?.profile?.maritalStatus).id : null;
    _selectedBloodGroupId = widget.profileProvider.profileModel?.data?.profile?.bloodGroup != null ? _bloodGroupList.firstWhere((bloodGroup) => bloodGroup.group == widget.profileProvider.profileModel?.data?.profile?.bloodGroup).id : null;
    _bloodDonor = (widget.profileProvider.profileModel?.data?.profile?.donateBloodInd ?? false) == 1;
    _diabetes = (widget.profileProvider.profileModel?.data?.profile?.diabetesInd ?? false) == 1;
    _htn = (widget.profileProvider.profileModel?.data?.profile?.htnInd ?? false) == 1;
    _asthma = (widget.profileProvider.profileModel?.data?.profile?.asthmaInd ?? false) == 1;
    print('gender id: $_selectedGenderId');
  }
  ...
  updateProfile(BuildContext context, ProfileProvider profileProvider) async {
    ...
    final _responseResult = profileProvider.profileModel?.data?.profile?.patientNoPk != null
    ? await profileProvider.updateProfile(
      id: profileProvider.profileModel?.data?.profile?.patientNoPk ?? 0,
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          TextButton(
            ...
            : () {},
            child:
            Text(profileProvider.profileLoaded ? "Save" : "", style: const TextStyle(color: kMediumBlueColor)),
          ),
        ],
      ),
                  ...
                  child: Stack(
                    children: [
                      WidgetFactory.buildProfileAvatar(
                          ...
                          radius: 100.0,
                          url: _uploadDocumentImage?.path ?? profileProvider.profileModel?.data?.profile?.image ?? 'assets/images/person.png',
                          imageType: _uploadDocumentImage == null ? (profileProvider.profileModel?.data?.profile?.image == null || profileProvider.profileModel?.data?.profile?.image == '' ? ImageType.Asset : ImageType.Network) : ImageType.File
                      ),
                      Positioned(
                        child: InkWell(
                          onTap: () {
                            if(profileProvider.profileModel?.data?.profile?.image != null && profileProvider.profileModel?.data?.profile?.image != '' || _uploadDocumentImage != null) {
                              getImageOptions(size.width, profileProvider);
                            } else {
                              getImage(size.width);
                            }
                          },
                ...
                Row(children: [
                  Expanded(
                    child: CustomTextInput(
                        ...
                        hint: "",
                        label: const Text.rich(TextSpan(text: 'First Name', style: kTitleTextStyle, children: [TextSpan(text: ' *', style: TextStyle(color: kRedColor))])),
                  ...
                  Expanded(
                    child: CustomTextInput(
                        ...
                        validator: validator.nameValidator,
                        hint: "",
                        label: const Text.rich(TextSpan(text: 'Last Name', style: kTitleTextStyle, children: [TextSpan(text: ' *', style: TextStyle(color: kRedColor))])),
                        textInputType: TextInputType.text,
                        textInputAction: TextInputAction.next),
                  ),
                ]),
                  ...
                  children: [
                    Expanded(
                      child: CustomTextInput(
                          ...
                          hint: "",
                          label: const Text.rich(TextSpan(text: 'Phone Number', style: kTitleTextStyle, children: [TextSpan(text: ' *', style: TextStyle(color: kRedColor))])),
                          ...
                          children: [
                            Expanded(
                                child: CustomTextInput(
                                    ...
                                    hint: "",
                                    label: const Text.rich(TextSpan(text: 'Date of Birth', style: kTitleTextStyle, children: [TextSpan(text: ' *', style: TextStyle(color: kRedColor))])),
                                    textInputType: TextInputType.text,
                                    textInputAction: TextInputAction.next)),
                            Padding(
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: DropdownButtonFormField<GenderModel>(
                          ...
                          hint: const Text.rich(TextSpan(text: 'Gender',
                            style: TextStyle(color: kDeepBlueColor), children: [
                              TextSpan(text: ' *', style: TextStyle(color: kRedColor))
                              ])
