...
class RecentAppointmentDetails extends StatefulWidget {
  final PatientInfoProvider provider;
  const RecentAppointmentDetails({Key? key, required this.provider}) : super(key: key);

  @override
  _RecentAppointmentDetailsState createState() => _RecentAppointmentDetailsState();
}

class _RecentAppointmentDetailsState extends State<RecentAppointmentDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: kWhiteColor,
        elevation: 0,
        title: const Text('Appointment Details', style: kAppBarBlueTitleTextStyle),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                if(provider.appointmentConfirmationModel?.data?.reportFeeInd == 0)
                  Padding(
                    padding:
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Expanded(
                            flex: 1,
                            child: Text("${provider.appointmentConfirmationModel?.data?.doctorFee ?? 0} Tk",
                                style: kTitleTextStyle, textAlign: TextAlign.end))
                      ],
                    ),
                  ),
                if(provider.appointmentConfirmationModel?.data?.reportFeeInd == 1)
                  Padding(
                    padding:
                    const EdgeInsets.only(left: 16.0, right: 16.0, bottom: 8.0),
                    child: Row(
                      ...
                      children: [
                        ...
                        Expanded(
                            flex: 1,
                            child: Text("${provider.appointmentConfirmationModel?.data?.reportFee ?? 0} Tk",
                                style: kTitleTextStyle, textAlign: TextAlign.end))
                      ],
                    ),
                  ),
                            ...
                            children: [
                              RichText(
                                text: TextSpan(
                                    ...
                                    children: [
                                      TextSpan(
                                          text: " ${int.parse(provider.appointmentConfirmationModel?.data?.reportFeeInd == 0 ? (provider.appointmentConfirmationModel?.data?.doctorFee ?? '0') : (provider.appointmentConfirmationModel?.data?.reportFee ?? '0')) + 10} Tk",
                                          style: const TextStyle(
                                            color: kOrangeColor,
                                          ))
                                    ]),
