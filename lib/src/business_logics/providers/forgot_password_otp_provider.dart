...
class ForgotPasswordOtpProvider extends ChangeNotifier {
  ...
  late ForgotPasswordOTPModel _forgotPasswordOTPModel;
  ...
  ForgotPasswordOTPModel get forgotPasswordOTPModel => _forgotPasswordOTPModel;
  ...
  Future<bool> verifyOTP(String phone, String otp) async {
    _inProgress = true;
    notifyListeners();
    final _response = await repository.verifyOTP(phone, otp);
    LogDebugger.instance.d(_response.object);
    if (_response.id == ResponseCode.SUCCESSFUL) {
      _inProgress = false;
      _forgotPasswordOTPModel = _response.object as ForgotPasswordOTPModel;
      notifyListeners();
      return true;
    } else {
      _inProgress = false;
